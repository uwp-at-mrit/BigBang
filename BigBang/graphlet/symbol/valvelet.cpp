...
void Valvelet::prepare_style(ValveStatus status, ValveStyle& s) {
	...
	case ValveStatus::Manual: {
		CAS_VALUES(s.mask_color, Colours::Teal, s.handle_color, Colours::DarkGray);
	}; break;
...
TValvelet::TValvelet(char tag, float radius, double degrees) : TValvelet(ValveStatus::Manual, tag, radius, degrees) {}
TValvelet::TValvelet(ValveStatus default_status, float radius, double degrees) : TValvelet(default_status, 'M', radius, degrees) {}

TValvelet::TValvelet(ValveStatus default_status, char tag, float radius, double degrees) : ISymbollet(default_status, radius, degrees) {
...
void TValvelet::prepare_style(ValveStatus status, ValveStyle& s) {
	switch (status) {
	case ValveStatus::Manual: {
...
void TValvelet::on_status_changed(ValveStatus status) {
	switch (status) {
	case ValveStatus::Unopenable: {
		if (this->bottom_up_mask == nullptr) {
			this->bottom_up_mask = polar_masked_sandglass(this->sgradius, this->degrees, 0.80);
...
void TValvelet::draw(CanvasDrawingSession^ ds, float x, float y, float Width, float Height) {
	...
	if (style.mask_color != nullptr) {
		...
		ds->FillGeometry(this->sign_body, scx, scy, style.mask_color);
		ds->DrawGeometry(mask, bcx, bcy, style.mask_color, default_thickness);
	} else {
		ds->FillGeometry(this->sign_body, scx, scy, style.body_color);
	}
