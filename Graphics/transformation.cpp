...
float3x2 make_translation_matrix(float x, float y) {
	return make_float3x2_translation(float2(x, y));
}

float3x2 make_rotation_matrix(float radians, float cx, float cy, float tx, float ty) {
...
float3x2 make_rotation_matrix(double degrees, float cx, float cy, float tx, float ty) {
