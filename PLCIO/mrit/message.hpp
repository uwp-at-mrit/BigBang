...
void mr_write_header(Windows::Storage::Streams::IDataWriter^ mrout, uint8 fcode, uint16 db_id, uint16 addr0, uint16 addrn);

void mr_read_tail(Windows::Storage::Streams::IDataReader^ mrin, uint16 size, uint8* data, uint16* checksum, uint16* eom);
void mr_write_tail(Windows::Storage::Streams::IDataWriter^ mrout, uint8* data, uint16 size);
