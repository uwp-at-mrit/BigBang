...
static void read_handle_reply_loop(IDataReader^ mbin, IDataWriter^ mbout, StreamSocket^ socket, Platform::String^ id) {
    ...
    create_task(mbin->LoadAsync(header_length)).then([=](unsigned int size) {
        ...
        try {
            ...
            read_handle_reply_loop(mbin, mbout, socket, id);
        } catch (Platform::Exception^ e) {
            rsyslog(e->Message);
            delete socket;
        } catch (task_canceled&) {
...
void ModbusListener::welcome(StreamSocketListener^ listener, StreamSocketListenerConnectionReceivedEventArgs^ e) {
    ...
    read_handle_reply_loop(mbin, mbout, client, id);
}
