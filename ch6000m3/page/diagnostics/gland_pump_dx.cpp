...
public:
	WaterPumpDx(GlandPumpDiagnostics* master, bool ps, unsigned int color) : master(master), ps(ps) {
...
GlandPumpDiagnostics::GlandPumpDiagnostics(PLCMaster* plc) : ISatellite(plc->get_logger(), __MODULE__), device(plc) {
...
void GlandPumpDiagnostics::on_tap_selected(IGraphlet* g, float local_x, float local_y) {
